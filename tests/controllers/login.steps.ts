...
defineFeature(feature, (test) => {
  ...
  test('login como artista', ({ given, when, then, and }) => {
    ...
    when(/^eu faço uma requisição POST para a url "(.*)" com JSON$/,
      async (url, body) => {
  ...
  test('login como moderador', ({ given, when, then, and }) => {
    ...
    and(/^"(.*)" está cadastrado$/, async (login) => {
      expect(mockGetModeradores().filter((moderador: ModeratorEntity) => moderador.login === login)).not.toBe([])
    })

    and(/^a senha cadastrada é "(.*)"$/, async (pass) => {
      expect(mockGetModeradores().filter((moderador: ModeratorEntity) => moderador.pass === pass)).not.toBe([])
    })
